...
def mod_bfact_all_atoms(atomlist, smooth_value):
    """
    ...
    *atomlist*
      list of atoms
    *smooth_value*
      +ve value : smoothen by value given
      -ve value : sharpen by value given
    """
    if smooth_value > 0.0:  # positive value
        for atm in atomlist:
            atm.temp_fac += smooth_value
    else:  # negative value
        for atm in atomlist:
            atm.temp_fac += smooth_value
...
def calc_map_density(mapin, structure_instance, verbose=0):
    """
    Returns a new map instance of electron density calculated from
    the given structure.
    Arguments:
    *mapin*
      input map for initialisation
    *structure_instance*
    ...
    if verbose >= 1:
        print('New map origin {0}'.format(newMap.origin))
