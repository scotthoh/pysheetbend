...
def RTop_orth(tgtArr, srcArr, weighted=False):
  ...
  if debug:
    ...
    print('eval ', eval)
    print('evev ', evec)
  rot_mat = Quaternion([evec[0,0], evec[1,0], evec[2,0], evec[3,0]]).to_rotation_matrix()
...
class Pseudoregularize:
  ...
  def regularize_frag(self, mol_work):
    ...
    for x in self.fragsList: #range(len(self.fragsList)): # list of fragments of atoms
      ...
      for r in range(len(frag_ref_CA)): # each key represent each residue
        ...
        for a in r_atms:  # loop atoms in current residue
          ...
          w1.append(w01)
          w2.append(w02)
        # end loop atoms in current residue
        #w0.append(w0tmp)
        #w1.append(w1tmp)
        #w2.append(w2tmp)
      ...
      for r in range(len(frag_ref_CA)):
        if r<2:
          ...
          for r1 in range(dr-r):
            slwork.append(frag_work.get_selection(frag_ref_CA[0].res_no, frag_ref_CA[0].res_no))
            slref.append(frag_ref.get_selection(frag_ref_CA[0].res_no, frag_ref_CA[0].res_no))
          ...
          r2e_resno = frag_ref_CA[r2_end].res_no
          slwork.append(frag_work.get_selection(r2s_resno, r2e_resno))
          slref.append(frag_ref.get_selection(r2s_resno, r2e_resno))
          for r1 in range(r+dr-r2_end):
            slwork.append(frag_work.get_selection(r2e_resno, r2e_resno))
            slref.append(frag_ref.get_selection(r2e_resno, r2e_resno))
          co1 = frag_work.combine_SSE_structures(slwork)
          co2 = frag_ref.combine_SSE_structures(slref)
        else:
          r2_start = max( min( r-dr, len(frag_ref_CA)-1 ), 0)
    ...
    '''
    Nf = len(self.fragsList)
    from sklearn import mixture
    # get gaussian components
    clf = mixture.GaussianMixture(n_components=Nf, covariance_type='full', n_init=5)
    vals = self.atomList.get_pos_mass_list()[:, :3]
    clf.fit(vals)
    clf.
    clf.eigs_ = np.zeros(clf.means_.shape)
    clf.eigvs_ = np.zeros(clf.covariances_.shape)
    # get the principal directions for the gaussian components
    for i, v in enumerate(clf.covariances_):
      u, v = np.linalg.eig(v)
      clf.eigs_[i, :] = u
      clf.eigvs_p[i, :] = v

    # get the principal axis for the chains
    moms = np.zeros((Nf, 3, 3))
    for i, f in enumerate(fragsList):
      x = f.get_pos_mass_list()[:, :3]
